...
public:
    ...
    virtual void wait_target(uint32 target) = 0;
    virtual void wait_target(uint32 target, uint32 target2) = 0;
};
...
private:
    ...
    enum { NORMAL, WAITING, TARGETING} m_state;
    ...
    void send_target(uint32 target);
...
public:
    ...
    virtual void wait_target(uint32 target);
    virtual void wait_target(uint32 target, uint32 target2);
};
